...
class ResourcePathProcessor
{
    ...
    public static function process(IService $service)
    {
        ...
        $requestSegments = $absoluteRequestUri->getSegments();
        $requestUriSegments = array_slice(
            $requestSegments,
            $absoluteServiceUri->getSegmentCount()
        );
        
        if(!$requestUriSegments && end($requestSegments) == ODataConstants::URI_METADATA_SEGMENT){
            $requestUriSegments = array_slice(
                $absoluteRequestUri->getSegments(),
                $absoluteServiceUri->getSegmentCount()-1
            );
        }
